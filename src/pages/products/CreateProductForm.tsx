...
const CreateProductForm = () => {
    ...
    const { register, handleSubmit, formState: { errors }, setValue, getValues, trigger } = useForm<FormSchemaType>({
        resolver: zodResolver(FormSchema)
    })
    const navigate = useNavigate();

    const createProduct = async (formData: FormSchemaType) => {
        try {
            setIsLoading(true)
            const res = await axiosInstance.post<ProductListModel>(`products`, formData)
            toast.success('Successfully created user data')
            navigate({ pathname: `/products/${res.data._id}`})
        }
        catch(ex){
